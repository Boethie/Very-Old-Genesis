...
public class DebrisBlocks extends VariantsCombo<MultiMetadata, BlockGenesisDebris, ItemBlockMulti<MultiMetadata>>
{
	...
	static
	{
		ArrayList<IMetadata<?>> list = new ArrayList<>();
		Iterables.addAll(list, Arrays.stream(EnumTree.values()).filter(EnumTree::hasDebris).collect(Collectors.toList()));
	...
	{
		super("debris",
				ObjectType.createBlock(MultiMetadata.class, "debris", BlockGenesisDebris.class),
				MultiMetadata.class, VARIANTS);
