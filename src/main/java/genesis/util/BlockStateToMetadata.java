...
public class BlockStateToMetadata
{
	...
	public static List<IProperty<?>> getSortedProperties(Collection<IProperty<?>> properties)
	{
		...
		if (output == null)
		{
			output = new ArrayList<>(properties);
			Collections.sort(output, (prop1, prop2) -> {
	...
	public static int getMetadataLeftAfter(IProperty<?>... properties)
	{
		...
		if (bitsLeft > 0)
		{
			return (int) Math.pow(2, bitsLeft);
		}
	...
	public static void serializeBlockState(IBlockState inputState, ByteBuf buf)
	{
		...
		propertyList.stream().filter(property -> metaState.getValue(property).equals(defaultState.getValue(property))).forEach(property -> buf.writeInt(getSortedValues(property).indexOf(inputState.getValue(property))));
	}
