...
import genesis.util.GenesisDataSerializers;
	...
	public void onUpdate()
	{
		...
		if (worldObj.isRemote)
		{
			...
			else
			{
				...
				if (horizSpeed > 0.1)
				{
					pitchTarget = (float) -Math.toDegrees(Math.atan2(diffY, horizSpeed));
				}
			}
			...
			{
				// TODO: check if full reassignment is correct here or should just be an offset
			...
			public IMessage onMessage(final MeganeuraUpdateMessage message, final MessageContext ctx)
			{
				...
				mc.addScheduledTask(() ->
				{
					...
					else
					{
						...
						{
							Genesis.logger.warn("entity: null, entityID: " + message.entityID);
						}
