...
public class WorldGenTreeDryophyllum extends WorldGenTreeBase
{
	...
	protected boolean doGenerate(World world, Random rand, BlockPos pos)
	{
		...
		int base = height - ((treeType == TYPE_SMALL_1 || treeType == TYPE_SMALL_2) ? 4 : 7);
	...
	private void generateBigTreeLeavesAndBranches(World world, BlockPos pos, Random rand)
	{
		for (int i = 0; i < 20; i++)
		{
			...
			double yr = 9;
			double rPart = 2.7;
			Vec3d currentPos = new Vec3d(pos).add(new Vec3d(
					(rand.nextDouble() * 2 - 1) * r, (rand.nextDouble()) * yr, (rand.nextDouble() * 2 - 1) * r
			));
			BlockPos tryPos = pos.down(rand.nextInt(5) + 2);
			double dy = currentPos.yCoord - pos.getY();
			if (abs(dy) > 9)
			{
				tryPos = tryPos.up((int) round(abs(dy) - 9));
			}
			
			generateLeaves(world, rand, currentPos, rPart);
			generateBranch(world, rand, tryPos, currentPos);
		}
	}
	...
	private void generateSmallTreeLeaves(World world, Random rand, BlockPos pos)
	{
		for (int i = 0; i < 8; i++)
		{
			...
			generateLeaves(world, rand, currentPos, rPart);
			generateBranch(world, rand, pos.down(rand.nextInt(4) + 1), currentPos);
		}
	}
	...
	private void generateBranch(World world, Random rand, BlockPos trunkPos, Vec3d endPos)
	{
		...
		while (curr.subtract(endPos).dotProduct(new Vec3d(1, 1, 1)) < 0)
		{
			...
			if (prev != null && prev.getX() == p.getX() && prev.getZ() == p.getZ() && p.getX() == nextBlock.getX() && p.getZ() == nextBlock.getZ())
			{
				prev = p;
				curr = next.addVector(rand.nextBoolean() ? -1 : 1, 0, rand.nextBoolean() ? -1 : 1);
				dir = endPos.subtract(curr).normalize();
			}
			else
			{
				prev = p;
	...
	{
		BlockPos endBlock = new BlockPos(endPos);
		if (endBlock.equals(pos))
		{
			return pos;
		}
		double dx = endPos.xCoord - pos.getX();
		double dz = endPos.zCoord - pos.getZ();
		dx = Math.signum(Math.round(dx));
		dz = Math.signum(Math.round(dz));
		if (endBlock.getX() == pos.getX())
		{
			return pos.add(0, 0, (int) dz);
		}
		if (endBlock.getZ() == pos.getZ())
		{
			return pos.add((int) dx, 0, 0);
		}
		return pos.add((int) dx, 0, (int) dz);
	}
	...
	{
		int total = max(hasSpaceCount(world, pos.up(), trunkPos), hasSpaceCount(world, pos.down(), trunkPos)) +
				max(hasSpaceCount(world, pos.east(), trunkPos), hasSpaceCount(world, pos.west(), trunkPos)) +
				max(hasSpaceCount(world, pos.north(), trunkPos), hasSpaceCount(world, pos.south(), trunkPos));
		return total < 2;
	}
	...
	private int hasSpaceCount(World world, BlockPos pos, BlockPos trunk)
	{
		...
		return (world.getBlockState(pos).getBlock() != wood.getBlock() ? 1 : 0);
	}
