...
/**
 * Created by Simeon on 9/14/2015. Heavily modified by Zaggy1024 on 11/10/2015.
 */
public class BlockTallTorch extends Block
{
	...
	public boolean canPlaceTorchAt(World world, BlockPos pos, Part part)
	{
		...
		for (EnumFacing facing : EnumFacing.values())
		{
			if (FACING.getAllowedValues().contains(facing)
					&& canTorchStay(world, pos, state.withProperty(FACING, facing), true))
			{
				return true;
			}
		}
	...
	{
		return canPlaceTorchAt(world, pos, Part.BOTTOM) || canPlaceTorchAt(world, pos.down(), Part.TOP);
	}

	@Override
	public IBlockState onBlockPlaced(World world, BlockPos pos, EnumFacing hitSide, float hitX, float hitY, float hitZ,
			int metadata, EntityLivingBase placer)
	{
		...
		case WEST:
			priority = new EnumFacing[]
			{ hitSide, hitSide.rotateYCCW(), hitSide.rotateY(), hitSide.getOpposite(), EnumFacing.UP };
			break;
		}
	...
	public boolean removedByPlayer(World world, BlockPos pos, EntityPlayer player, boolean willHarvest)
	{
		...
		if (other != null)
		{
			world.markAndNotifyBlock(other, world.getChunkFromBlockCoords(other), otherState,
					world.getBlockState(other), 1);
		}
	...
	public MovingObjectPosition collisionRayTrace(World world, BlockPos pos, Vec3 start, Vec3 end)
	{
		...
		case NORTH:
			...
			bb = bb.addCoord(toOuter * -facing.getFrontOffsetX(), toOuter * -facing.getFrontOffsetY(),
